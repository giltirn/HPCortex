...
bool GraphInitialize::operator==(const GraphInitialize &r) const{
  if(nnode != r.nnode || global_attr_size != r.global_attr_size || batch_size != r.batch_size || node_attr_sizes.size() != r.node_attr_sizes.size() || edge_attr_sizes.size() != r.edge_attr_sizes.size() || edge_map.size() != r.edge_map.size()) return false;
