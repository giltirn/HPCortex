...
template<typename FloatType, int TensDim, typename InputType, typename Store, typename ActivationFunc>
void BatchTensorDNNlayer<FloatType,TensDim,InputType,Store,ActivationFunc>::deriv(Vector<FloatType> &cost_deriv, int off, Tensor<FloatType,TensDim> &&_above_deriv, InputType* input_above_deriv_return) const{
...
template<typename FloatType, int TensDim, typename InputType, typename Store, typename ActivationFunc>
void BatchTensorDNNlayer<FloatType,TensDim,InputType,Store,ActivationFunc>::update(int off, const Vector<FloatType> &new_params){
  cpt.update(off,new_params);
  leaf.v.update(off+cpt.nparams(),new_params);
}

template<typename FloatType, int TensDim, typename InputType, typename Store, typename ActivationFunc>
void BatchTensorDNNlayer<FloatType,TensDim,InputType,Store,ActivationFunc>::step(int off, const Vector<FloatType> &derivs, FloatType eps){
  cpt.step(off,derivs,eps);
  leaf.v.step(off+cpt.nparams(),derivs,eps);
}

template<typename FloatType, int TensDim, typename InputType, typename Store, typename ActivationFunc>
void BatchTensorDNNlayer<FloatType,TensDim,InputType,Store,ActivationFunc>::getParams(Vector<FloatType> &into, int off){
