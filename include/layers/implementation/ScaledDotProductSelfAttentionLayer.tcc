...
template<typename Config, typename InputType, typename Store>
int ScaledDotProductSelfAttentionLayer<Config,InputType,Store>::deriv(Vector<FloatType> &cost_deriv, int off, Tensor<FloatType,3> &&_above_deriv, InputType* input_above_deriv_return) const{
...
template<typename Config, typename InputType, typename Store>
int ScaledDotProductSelfAttentionLayer<Config,InputType,Store>::update(int off, const Vector<FloatType> &new_params){
  attentionQKV.update(off,new_params);
  return leaf.v.update(off + attentionQKV.nparams(),new_params);
}
  
template<typename Config, typename InputType, typename Store>
int ScaledDotProductSelfAttentionLayer<Config,InputType,Store>::step(int off, const Vector<FloatType> &derivs, FloatType eps){
...
template<typename Config, typename InputType, typename Store>
int ScaledDotProductSelfAttentionLayer<Config,InputType,Store>::getParams(Vector<FloatType> &into, int off) const{
