...
/**
 * @brief GEMM (row-major): alpha * op(A) * op(B) + beta * C 
...
/**
 * @brief Batched GEMM (row-major): alpha * op(A[i]) * op(B[i]) + beta * C[i]    for i in 0...batchCount-1
 * @param transa operation performed on A
 * @param transb operation performed on B
 * @param m rows of C
 * @param n columns of C
...
/**
 * @brief Batched GEMV (row-major) :  alpha * op(A[i]) * x[i] + beta * y[i]    for i in 0...batchCount-1
 ...
 */
template<typename FloatType>
void cmBatchedGEMM(BLASop transa,
                 BLASop transb,
		 int m, int n, int k,
		 const FloatType           *alpha,
 ...
 */
template<typename FloatType>
void cmBatchedGEMV(BLASop trans,
		 int m, int n,
		 const FloatType           *alpha,
		 const FloatType           *A, int lda,
		 long long int         strideA,
 ...
 */
template<typename FloatType>
void cmGEMM(BLASop transa, BLASop transb,
	  int m, int n, int k,
	  const FloatType           *alpha,
	  const FloatType           *A, int lda,
	  const FloatType           *B, int ldb,
