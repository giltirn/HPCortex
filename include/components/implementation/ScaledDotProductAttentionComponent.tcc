...
Tensor<typename Config::FloatType,3> ScaledDotProductAttentionComponent<Config>::value(const Tensor<FloatType,3> &Q, const Tensor<FloatType,3> &K, Tensor<FloatType,3> &V, EnableDeriv enable_deriv){
  ...
  Tensor<FloatType,3> S = mulQKtoGetS.value(Q,K, enable_deriv);
  Tensor<FloatType,3> SS = softmaxS_to_SS.value(S, enable_deriv);
  return mulSSVtoGetOut.value(SS,V, enable_deriv);
}
template<typename Config>
